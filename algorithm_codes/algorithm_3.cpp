...
int main(int argc, char const *argv[])
{
	...
	file_name.str(string());
	file_name << tunnel_name << my_rank << "_2.txt";
	fout.open(file_name.str().c_str(), ios_base::app);
	output(R, fout);
	fout.close();
	R.clear();
	...
	if(!my_rank)
	{
		...
		{
			file_name.str(string());
			file_name << tunnel_name << i << "_2.txt";
			fin.open(file_name.str().c_str());
			gather_result(fin);
			fin.close();
		}
		...
		FOR_MAP(ii,R)
		{
			if (ii->second >= (MIN_SUP * num_procs))
			{
				temp[ii->first]=ii->second;
			}
		}
